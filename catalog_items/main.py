...
from db_setup import Base, Components, PartItem, UserInventory
...
# Source: https://docs.sqlalchemy.org/en/latest/dialects/sqlite.
"""
engine = create_engine('sqlite:///pcinventory.db',
...
class ReuseForm(Form):
    ...
    pic_ctrl = FileField('Image File', validators=[DataRequired()])
    ...
    description = TextAreaField('Part Description ',
                                validators=[DataRequired()])
    pic_ctrl_part = FileField('Image File', validators=[DataRequired()])
    cost = IntegerField('Price ', validators=[DataRequired()])
    qty = IntegerField('Unit Quantity ', validators=[DataRequired()])
...
def gconnect():
    ...
    h = httplib2.Http()
    response = h.request(url, 'GET')[1].decode('utf-8')
...
def createUser(login_session):
    ...
    newUser = UserInventory(name=login_session['username'],
                   email=login_session['email'],
                   picture=login_session['picture'])
    session.add(newUser)
    session.commit()
    usr = session.query(UserInventory).filter_by(email=login_session['email\
                                                             ']).one_or_none()
    return usr.id
...
def getUserInfo(user_id):
    ...
    usr = session.query(UserInventory).filter_by(id=user_id).one_or_none()
    return usr
...
def getUserID(email):
    ...
    try:
        usr = session.query(UserInventory).filter_by(email=email).one_or_none()
        return usr.id
    except:
        return None
...
def showComponents():
    ...
    return render_template('components.html', pc_components=pc_components, pc_parts=pc_parts)


"""
CREATE NEW COMPONENT ROUTE (/components/new)
"""
# Correction Source: https://discussions.udacity.com/t/solved-keyerror-user-id/223049/3
...
def newComponent():
    ...
    if newComponentItem.user_id != login_session.get('user_id'):
        ...
        if request.method == 'POST':
            ...
            file = request.files['pic_ctrl']
            #pic_nme = secure_filename(file.filename)
            ...
            if request.form['name']:
                ...
                session.commit()
                """Informs the user that a new component is added successfully"""
            ...
            return render_template('newComponent.html', form=form)
            return redirect(url_for('showComponents'))
            # https://stackoverflow.com/questions/46822068/how-to-reload-flask-based-webpage-after-form-submission
...
def editComponent(pccomponents_id):
    form = ReuseForm(request.form)
    editedComponent = session.query(Components).filter_by(
        id=pccomponents_id).one()
    ...
    if editedComponent.user_id != login_session.get('user_id'):
        ...
        if request.method == 'POST':
            ...
            fileEdit = request.files['pic_ctrl']

            form.validate_on_submit()
            if request.form['ec_name']:
                editedComponent.c_name = request.form['ec_name']
                pic_name = fileEdit.filename
                pic = fileEdit.read()
                editedComponent.pic = pic
                session.commit()
...
def deleteComponent(pccomponents_id):
    deleteTheComponent = session.query(Components).filter_by(
        id=pccomponents_id).one_or_none()
...
def showParts(pccomponents_id):
    pc_components = session.query(Components).filter_by(
        id=pccomponents_id).one_or_none()
    admin = getUserInfo(pc_components.user_id)
    pc_parts = session.query(PartItem).filter_by(
        pccomponents_id=pccomponents_id).all()
    print(admin)  # Identify if user is authorized
    return render_template('pcParts.html', pc_parts=pc_parts, pc_components=pc_components, admin=admin)
...
def newPartItem(pccomponents_id):
    ...
    if login_session.get('user_id') != pc_components.user_id:
        ...
        if request.method == 'POST':
            ...
            filePart = request.files['pic_ctrl_part']

            form.validate_on_submit()
            if request.form['prt_name']:
                newPart = PartItem(p_name=form.prt_name.data,
                                   picpart_name = filePart.filename,
                                   picpart=filePart.read(),
    ...
    if login_session.get('user_id') != pc_components.user_id:
        ...
        if request.method == 'POST':
            ...
            if request.form['prt_name']:
                ...
                picpart_name = filePart.filename
                picpart = filePart.read()
                editThePartItem.picpart = picpart
